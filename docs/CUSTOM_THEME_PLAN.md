...
**Status:** Phases 0–4 (high-value subset) DONE — `18.0` tagged **`v18.0.1.2.1`** (`1ccea43`, 2026-06-14 feature: customizable app-launcher (9-dots) background — palette-following + admin override via the Theme dialog — plus the AR Google-Fonts help-text fix; prior `v18.0.1.1.1` `ffb4b55` = bug-fix pass: RTL flip / submenu palette-follow / Discuss dark / dark-text+kanban numbers / font-tab move; `v18.0.1.1.0` = comprehensive theme panel + real dark skin); **Phase-4 "everything" tier DONE 2026-06-14** (EN/AR tour matrix in CI, 19.0-readiness notes, per-addon READMEs); only deferred: Phase-1 doc re-capture | **Date:** 2026-06-11 | **Targets:** Odoo 18.0 Community | **Products:** Sudan Medical-Supply ERP demo (`erpmedsupply`, Arabic-first EN/AR), CMP, ePHEM
...
> show dead Spiffy UI). The Phase-4 "everything" tier (EN/AR tour matrix in CI, 19.0-readiness notes,
> per-addon READMEs) shipped 2026-06-14 (odoo-nile-theme `28c97b2`).
...
| **4 — Hardening & distribution** 🟡 **HIGH-VALUE SUBSET DONE 2026-06-13.** DONE: Hoot JS unit-test scaffold (`nile_config/static/tests/color_picker.test.js` + `web.assets_unit_tests`, 12/12 in the in-browser runner) + CI `js-tests` lane (`.github/scripts/run_hoot.py`); version bump → `18.0.1.1.0`; release tag `v18.0.1.1.0`; tag-based deploy pinning (`erpmedsupply-addons` `v18.0.1.0.0` + `docs/DEPLOY_PINS.md` + runbook §5). **"EVERYTHING" TIER DONE 2026-06-14** (odoo-nile-theme `28c97b2`): EN/AR critical-flow tour matrix per brand in CI (`.github/scripts/run_tours.py` + ci.yml `tours` job, verified live on `erpmedsupply` EN+AR), `UPGRADE_19.md` 19.0-readiness checklist keyed to TOUCHPOINTS, and the last missing per-addon README (`nile_config`). | **6 pd** | DONE: prod clones consume pinned tags; install + contrast CI green; JS + tour CI lanes added (pending first GitHub-Actions run) |

**Total: ~36 person-days.** Phases 0–4 are DONE (`18.0` @ `v18.0.1.2.1`) — the ONLY remaining plan item is the deferred Phase-1 doc re-capture (manual + both decks), ON HOLD until the user asks.
